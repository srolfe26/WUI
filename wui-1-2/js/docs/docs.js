...
(function($,window) {
        ...
        $.extend(me, new Wui.O, {
            ...
            processCode:        function(code){
                                    ...
                                    code.code.replace(/(\/\*\*((?:[^\*]|(?:\*+(?:[^\*/])))*)\*+\/[\s]*([^\:]+)\:[\s]*function)/g,function(m,all,comment,itemName,charIndex){
                                        addToConglomerate($.trim(itemName),comment,me,'method');
                                        return '';
                                    });
    ...
    Wui.transformJavaDoc = function(documentation){
        ...
        var htmlSafeText = Wui.HTMLifyCode(m.replace(/\*/g,''),false),
            keepPre = false;
    ...
    Wui.HTMLifyCode = function(c,codeColoring,lang){
        ...
        var e = document.createElement('i'),
            ...
            var r = '<pre '+addClass+ '><code '+addClass+ '>' + $.trim(Wui.escapeTags(c)) + '</code></pre>';
        ...
        var wuiCode = $('.wui-doc-code');
            if(wuiCode.length > 0){
                ...
                wuiCode.each(function(){
                    var itm     = $(this),
                        docLang = (itm.prop('tagName').toLowerCase() == 'style') ? 'css' : 'javascript',
                        docCode = itm.text();

                    preObj.append($(Wui.HTMLifyCode(docCode,true,docLang)));
                });
                
                // create the <pre> object with associated button to show and hide it
                var testObj     = $('<div>').attr({id:'wui-unit-tests'}),
    ...
    Wui.assert = function(descrip,test,count,hideFn){
        ...
        var startTime   = new Date(),
            stringVal   = (fnString.length && hideFn !== true) ? Wui.HTMLifyCode(fnString,false) : '',
            endTime     = new Date(),
            testNum     = (count) ? count : '-',
    ...
    Wui.splay = function(o){
        ...
        for(i in o){
            var itm = {element:''};
            if(typeof o[i] == 'function')   { itm.element = 'method ' + i + '<br />' + Wui.HTMLifyCode(o[i].toString(),false); }
            else                            { itm.element = i; }
            objData.push(itm);
        };
