...
// const ERROR_ICON = '<svg width="24" height="24" viewBox="0 0 24 24"><path d="M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2v-4h2v4z"/></svg>';

// // HELPER FUNCTIONS
// function _eventFire(el: HTMLElement, etype: string): void {
...
// function _outerHeightMargins(el: HTMLElement): number {
...
//       this.close();
//     } else {
//       _eventFire(this.input, 'mousedown');
//     }
//   }
...
//     const isSearchable = override !== undefined ? override : this.items.length >= this.searchThreshold || !this.searchLocal;
...
//     if (lockIt === true && _outerHeightMargins(body) > verge.viewportH()) {
...
//       width = width > widestChild || widestChild + this.input.getBoundingClientRect().left > verge.viewportW() ? width : widestChild;
...
//     if (isset(this.value) && this.value.disabled !== true && text.length === 0 && String(this.value[this.valueItem]).trim().length === 0) {
...
//         group.childNodes.forEach((child) => (visCount += _isVisible(child) ? 1 : 0));
...
//   selectAjacent(dir: number) {
//     // Determine the visible elements.
//     const options = this.items
//       .map((item) => {
//         if (item.record.disabled === true || !_isVisible(item.el)) {
//           return undefined;
//         } else {
//           return item.el;
//         }
...
//       if (item.textContent && item.textContent.toUpperCase().indexOf(srchVal.toUpperCase()) >= 0 && typeof foundFn === 'function') {
//         foundFn(item);
...
//   private itemSelect(item: HTMLElement | null, silent?: boolean) {
//     const selectedElement = this.dd.querySelector('.' + SELECTED_CLASS);
...
//       this.selected = [{ el: item, tswuiO: item }];
...
//     const optGroups: Record<string, HTMLElement> = {};
